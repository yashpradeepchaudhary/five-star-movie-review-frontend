...
export const getMovies = async (pageNo, limit) => {
  ...
  try {
    const { data } = await client(
      `/movie/movies?pageNo=${pageNo}&limit=${limit}`, // Where 'movie' comes from api/movie and "movies" comes from router
      {
        headers: {
          authorization: "Bearer " + token,
          "content-type": "multipart/form-data",
...
export const deleteMovie = async (id) => {
  ...
  try {
    const { data } = await client.delete(
      `/movie/${id}`, // Where 'movie' comes from api/movie and "movies" comes from router
      {
        headers: {
          authorization: "Bearer " + token,
        },
...
export const searchMovieForAdmin = async (title) => {
  ...
  try {
    const { data } = await client(
      `/movie/search?title=${title}`, // Where 'movie' comes from api/movie and "movies" comes from router
      {
        headers: {
          authorization: "Bearer " + token,
        },
